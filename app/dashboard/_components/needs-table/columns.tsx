...
export const columns: ColumnDef<NeedItemTableRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className='font-mono font-medium text-xs text-[var(--ink)]'>
          {target_quantity}{' '}
          <span className='text-[var(--muted)] font-sans'>{unit}</span>
        </span>
      )
    }
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span
          className={`inline-flex items-center px-2 py-0.5 rounded-[var(--radius-xs)] border text-xs font-medium ${config.className}`}
        >
          {config.label}
        </span>
      )
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        ...
        >
          <span className='rounded-[var(--radius-xs)] bg-[var(--background)] px-2 py-0.5 text-xs font-medium text-[var(--ink)] border border-[var(--border)]'>
    ...
    cell: ({ row }) => {
      ...
      return (
        <Link
          ...
          className='inline-flex items-center rounded-[var(--radius-xs)] border border-[var(--border)] bg-[var(--surface)] px-2.5 py-1 text-xs font-medium text-[var(--ink)] shadow-2xs transition-colors hover:bg-[var(--background)] focus:outline-none focus:ring-1 focus:ring-[var(--focus)]'
        >
          Editar
        </Link>
      )
