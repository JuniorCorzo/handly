...
export function PledgeButton({ needItemId, disabled }: PledgeButtonProps) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      data-need-id={needItemId}
      className="w-full rounded-[var(--radius-sm)] bg-[var(--primary)] px-4 py-2.5 text-sm font-semibold text-white transition-opacity duration-150 hover:opacity-90 focus:ring-2 focus:ring-[var(--focus)] focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-60"
    >
      Pledge to help
    </button>
  );
