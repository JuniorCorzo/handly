...
export function NeedItemForm({
  ...
  const selectedPointIds = defaultValues?.collection_point_ids ?? []

  // Redireccionar al panel principal tras guardar con éxito
  useEffect(() => {
    if (state?.success) {
  ...
  return (
    <form action={formAction} noValidate className='flex flex-col gap-5'>
          ...
          <div className='mt-2 rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--background)] p-3'>
            ...
            <div className='mt-1.5 flex gap-2'>
              <input
                ...
                className='flex-1 rounded-[var(--radius-xs)] border border-[var(--border)] bg-[var(--surface)] px-2.5 py-1.5 text-xs text-[var(--ink)] focus:outline-none focus:ring-1 focus:ring-[var(--focus)]'
              />
              <button
                type='button'
      ...
      {/* Collection Points Multi-select */}
        ...
        {collectionPoints.length === 0 ? (
          ...
          <div className='flex flex-col gap-2'>
            {collectionPoints.map((cp) => (
              <label
                key={cp.id}
                className='flex items-start gap-2 text-sm text-[var(--ink)] cursor-pointer'
              >
                <input
                  type='checkbox'
                  name='collection_point_ids'
                  value={cp.id}
                  defaultChecked={selectedPointIds.includes(cp.id)}
      ...
      <div className='flex items-center gap-3 pt-2'>
        <button
          ...
          className='flex-1 inline-flex items-center justify-center rounded-[var(--radius-sm)] bg-[var(--primary)] px-4 py-2.5 text-sm font-semibold text-white shadow-xs transition-opacity hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-[var(--focus)] disabled:cursor-not-allowed disabled:opacity-60'
        >
        ...
        <Link
          href='/dashboard'
          className='inline-flex items-center justify-center rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--surface)] px-4 py-2.5 text-sm font-medium text-[var(--ink)] shadow-2xs transition-colors hover:bg-[var(--background)] focus:outline-none focus:ring-2 focus:ring-[var(--focus)]'
        >
          Cancelar
        </Link>
      </div>
      ...
      {state?.success && (
        <p
          role='status'
          aria-live='polite'
          className='text-center text-sm font-medium text-green-700 bg-green-50 border border-green-200 rounded-[var(--radius-sm)] py-2'
        >
