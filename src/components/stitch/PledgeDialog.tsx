...
export function PledgeDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : 'No se pudo confirmar el compromiso.'
      )
    } finally {
      setSubmitting(false)
    }
    ...
    >
      ...
      <div className='relative flex max-h-[90dvh] w-full max-w-md flex-col overflow-hidden rounded-t-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] shadow-[0_4px_12px_oklch(0.23_0.02_173/0.10)] sm:rounded-[var(--radius-md)]'>
        <button
          ...
          aria-label='Cerrar diálogo'
          onClick={() => onOpenChange(false)}
          className='absolute right-3 top-3 inline-flex h-11 w-11 items-center justify-center rounded-full text-[var(--muted)] hover:bg-[var(--background)] hover:text-[var(--ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-2'
        >
          <svg
            aria-hidden='true'
            width='20'
        ...
        <div className='border-b border-[var(--border)] px-6 pb-4 pt-6'>
          {urgencyLabel ? (
            <p className='mb-2 text-xs font-semibold uppercase tracking-wider text-[var(--critical)]'>
              {urgencyLabel}
            </p>
          ) : null}
          <h2
            id={titleId}
            className='pr-10 text-xl font-semibold leading-tight text-[var(--ink)] [text-wrap:balance]'
          >
            {title}
          </h2>
        ...
        <div className='flex flex-col gap-6 overflow-y-auto p-6'>
          <div className='flex flex-col gap-2'>
            ...
            <div className='relative flex items-center'>
              <button
                ...
                className='absolute left-0 top-0 bottom-0 inline-flex w-11 items-center justify-center border-r border-[var(--border)] bg-[var(--surface)] text-[var(--muted)] hover:text-[var(--primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-2'
              >
                <svg
                  aria-hidden='true'
                  width='18'
              ...
              <input
                ...
                className='h-12 w-full border border-[var(--border)] bg-[var(--surface)] px-14 text-center font-mono text-lg text-[var(--ink)] focus:border-[var(--primary)] focus:outline-none focus:ring-1 focus:ring-[var(--primary)]'
              />
              <button
                type='button'
                aria-label='Aumentar cantidad'
                onClick={() => setQuantity((q) => clamp(q + 1))}
                className='absolute bottom-0 right-0 top-0 inline-flex w-11 items-center justify-center border-l border-[var(--border)] bg-[var(--surface)] text-[var(--muted)] hover:text-[var(--primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-2'
              >
                <svg
                  aria-hidden='true'
                  width='18'
          ...
          <div className='flex flex-col gap-3 border-t border-dashed border-[var(--border)] pt-4'>
            <button
              ...
              className='inline-flex min-h-[44px] w-full items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-[var(--primary)] px-4 py-3 text-sm font-semibold text-[var(--surface)] hover:bg-[var(--primary)]/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-2 disabled:opacity-60'
            >
