type NotificationKind =
  'critical' | 'urgent' | 'standard' | 'success' | 'reminder'
  ...
  return (
    <li className='flex items-start gap-4 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] p-4 transition-colors hover:bg-[var(--background)]'>
      ...
      <div className='min-w-0 flex-1'>
        ...
        <p className='mt-1 text-sm leading-relaxed text-[var(--muted)] [text-wrap:pretty]'>
          {body}
        </p>
        {actionLabel && onAction ? (
          <button
            type='button'
            onClick={onAction}
            className='mt-3 inline-flex min-h-[44px] items-center rounded-[var(--radius-sm)] bg-[var(--primary)] px-4 py-2 text-sm font-semibold text-[var(--surface)] hover:bg-[var(--primary)]/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-2'
          >
            {actionLabel}
          </button>
        ) : null}
