  ...
  return (
    <div className="flex items-center justify-between gap-4 border-b border-[var(--border)] p-4 last:border-b-0 hover:bg-[var(--background)]">
      <div className="min-w-0">
        ...
        {description ? (
          <p className="mt-0.5 text-sm leading-snug [text-wrap:pretty] text-[var(--muted)]">
            {description}
          </p>
        ) : null}
      </div>
...
export function SettingsLink({
  ...
  return (
    <a
      href={href}
      onClick={onClick}
      className="flex items-center justify-between p-4 text-sm text-[var(--ink)] hover:bg-[var(--background)] focus-visible:ring-2 focus-visible:ring-[var(--focus)] focus-visible:ring-offset-2 focus-visible:outline-none"
    >
      <span>{label}</span>
      <svg
        aria-hidden="true"
